...
def find_all_tokens(token_str: str, vocab):
    """
...
def gen_translation_task(df, vocab, cfg):
    """
    ...
    dataset = []
    for ind in tqdm(range(len(df_dest))):
        ...
        for idx, (df_idx, row) in enumerate(sample.iterrows()):
            if idx < k-1:
                ...
                in_token_str = row[src_lang]
                out_token_str = row[dest_lang]
                out_token_ids = find_all_tokens(out_token_str, vocab)
                latent_token_str = row[latent_lang]
                latent_token_ids = find_all_tokens(latent_token_str, vocab)
